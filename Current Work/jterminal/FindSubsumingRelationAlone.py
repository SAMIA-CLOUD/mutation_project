        ...
        for root, dirnames, filenames in os.walk(self.globalPath):
            for filename in fnmatch.filter(filenames, "*.java"):
                if str(filename) == "original.java":
                    continue
                if "Vt100TerminalModel.java" not in root:
                    continue
                newMutant = Mutant()
...
class MutantSubsumptionGraphNode(object):
    ...
    def __init__(self, predicted=False):
        self.id = next(MutantSubsumptionGraphNode.newID)
        self.outEdges = list()
        self.inEdges = list()
        self.mutants = set()
...
class MutantSubsumptionGraphEdge(object):
    ...
    def __init__(self, subsuming=None, subsumed=None):
        self.id = next(MutantSubsumptionGraphEdge.newID)
...
if __name__ == "__main__":
    ...
    dot_file_name=projectName + ".dot";
    with open(dot_file_name, "w") as dotFile:
        dotFile.write(fSet.mutantSubsumptionGraph.dotGraph())
    os.system("dot -Tbmp "+dot_file_name+" -o subsumption_graph.bmp");
