...
type='2';
mu_set='3'
writer=open("MinimalTestSuite_Delta"+type+"_mu_"+mu_set+".txt",'w');
writer_killed=open("Killed_MinimalTestSuite_Delta"+type+"_mu_"+mu_set+".txt",'w');
writer.write("Minimal Test Suite by Delta"+type+" and for mu"+mu_set+":\n");
writer_killed.write("Killed Mutants by Minimal Test Suites by Delta"+type+" and for mu"+mu_set+":\n");
trial=0;
ALL_SETS=[];
set_number=0;
while trial<5000:
	...
	mutants=[];
	print(trial);
	filename='difsets_delta'+type+'_u'+mu_set+'.csv';
	file=open(filename);
	lines=file.readlines();
